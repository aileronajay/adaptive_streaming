...
//	  Log.d("BEEJEE", "determineIdealFormat called in Format Evaluator");
	...
	public static final class KMAdaptiveEvaluator implements FormatEvaluator {
		...
		private final double a1 = 0.75;
		private final double a2 = 0.33;
		private final double a3 = 0.55;
		private final double a4 = 0.75;
		private final double a5 = 0.9;

		private final long b_min = 1 * 1000;
		private final long b_low = 2 * 1000;
		private final long b_high = 5 * 1000;
		private final long b_opt = 1 * 1000;
		private final double delta_b = 0.1 * 1000;
		private boolean isFastStartRunning = true;
		private final long T = 1;
		private double lastEstimate = 0;
		private boolean isBrMinIncreasing = false;

		/**
		 * @param bandwidthMeter Provides an estimate of the currently available bandwidth.
		 */
		public KMAdaptiveEvaluator(BandwidthMeter bandwidthMeter) {
			this (bandwidthMeter, DEFAULT_MAX_INITIAL_BITRATE,
					DEFAULT_MIN_DURATION_FOR_QUALITY_INCREASE_MS,
					DEFAULT_MAX_DURATION_FOR_QUALITY_DECREASE_MS,
					DEFAULT_MIN_DURATION_TO_RETAIN_AFTER_DISCARD_MS, DEFAULT_BANDWIDTH_FRACTION);
		...
		public void evaluate(List<? extends MediaChunk> queue, long playbackPositionUs,
			...
			int br_t = evaluation.format.bitrate;
			int r_t_next = evaluation.format.bitrate;
			int r_t_prev = evaluation.format.bitrate;

			int cur_format_index = 0;
			for(int i=0; i<formats.length; i++) {
				Format format = formats[i];
				if(evaluation.format.id == format.id) {
					cur_format_index = i;
				}
			}

			if(cur_format_index < formats.length) {
				r_t_prev = formats[cur_format_index+1].bitrate;
			} else {
				r_t_prev = formats[cur_format_index].bitrate;
			}

			if(cur_format_index >  0) {
				r_t_next = formats[cur_format_index-1].bitrate;
			} else {
				r_t_next = formats[cur_format_index].bitrate;
			}


			int br_min = formats[formats.length-1].bitrate;
			int br_max = formats[0].bitrate;

			double br_estimate = bandwidthMeter.getBitrateEstimate();
			double br_change = lastEstimate - bandwidthMeter.getBitrateEstimate();
			lastEstimate = br_estimate;
			if(lastEstimate > br_estimate) {
				isBrMinIncreasing = true;
			}

			long b_t = 0;


			long[] result = KMAlgorithm(br_t, br_max, br_min, r_t_next, r_t_prev, isBrMinIncreasing, b_t, br_estimate, br_change);
			for(int i=0; i<formats.length; i++) {
				Format format = formats[i];
				if(evaluation.format.bitrate == result[0]) {
					Log.d("BITRATE_SELECTED" , ""+format.bitrate);
		...
		//This code is implemented from the psuedocode mentioned in the KM algorithm paper
		//http://ieeexplore.ieee.org/stamp/stamp.jsp?arnumber=6229732&tag=1
		private long[] KMAlgorithm(int br_t, int br_max, int br_min, int r_t_next, int r_t_prev, boolean isBrMinIncreasing, long b_t, double br_estimate, double br_change) {

			int br_tplus1 = br_t;
			long b_delay = 0;
			if(isFastStartRunning == true && br_t != br_max && isBrMinIncreasing == true && br_t <=a1*br_estimate ){
				if(b_t < b_min){
					if(r_t_next <= a2*br_estimate){
						br_tplus1 = r_t_next;
					}
				} else if(b_t < b_low){
					if(r_t_next <= a3*br_estimate){
						br_tplus1 = r_t_next;
					}
				} else {
					if(r_t_next <= a4*br_estimate){
						br_tplus1 = r_t_next;
					}
					if(b_t > b_high){
						b_delay = b_high - T;
				...
				isFastStartRunning = false;
				if(b_t < b_min){
					br_tplus1 = br_min;
				} else if(b_t < b_low){
					if(br_t != br_min && br_t >= br_change){
						br_tplus1 = r_t_prev;
					}
				} else if(b_t < b_high){
					if(br_t == br_max || r_t_next >= a5*br_estimate){
						if(b_t - T > b_opt){
							b_delay = b_t - T;
						} else{
							b_delay = b_opt;
